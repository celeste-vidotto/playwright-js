...
//arrange
    ...
    await OpenAdmin.AdminPageLoaged(); //verifico que el modulo admin se haya cargado
...
//act
  await page.getByRole('textbox').nth(0).fill(camposDeFiltro.username);  //username
 ...
 // await page.getByRole('option', { name: 'Admin' }).click(); //user role
  await page.getByRole('textbox', { placeholder: 'Type for hints...' }).fill(camposDeFiltro.employeeName); //Employee Name
  await page.getByText('listbox').filter({ hasText: camposDeFiltro.employeeName }).click(); //employee name
