...
//arrange
    ...
    const camposDeFiltro = {
        username: user.filtroUserEmpty.username,
        userRole: user.filtroUserEmpty.userRole,
        employeeName: user.filtroUserEmpty.employeeName,
        status: user.filtroUserEmpty.status
    };
...
//act
  ...
  await expect(page.getByText('Error Invalid Parameter')).toBeVisible();
